...
router.get(`/api/${API_VERSION}/stats`, authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Sum up costs
    const totalCostThisMonth = (monthlyCost.data || []).reduce((s: number, sp: Record<string, unknown>) => s + parseFloat((sp.cost_usd as string) || '0'), 0);
